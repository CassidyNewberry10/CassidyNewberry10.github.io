//GRAPH 4: Base Pay by Senority, Gender, Job Type
//NEED TO ADD SHAPE CHANGING BASED ON JOB TYPE 
...
const titleText = 'Base Salary by Seniority, Gender, and Job Type';

const render = data => {
    const xValue = d => d.Seniority;
    const yValue = d => d.BasePay;
    const Gender= d => d.Gender;
    const JobType= d => d.Category;
...
d3.csv('SenorityBasePayGenderJob.csv').then(data => { 
    data.forEach(d =>{
        d.Seniority= +d.Seniority;
        d.BasePay= +d.BasePay;
        d.Gender= d.Gender;
        d.JobType= d.Category;
    })
   render(data);
